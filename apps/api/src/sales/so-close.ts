...
export async function handle(event: APIGatewayProxyEventV2){
  ...
  if (!["fulfilled","partially_fulfilled","cancelled"].includes(String(so.status))) return json(409,{message:`Cannot close when status is ${so.status}`});
  
  // Release leftover reserved on partials
