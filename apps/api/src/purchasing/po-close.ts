...
export async function handle(event: APIGatewayProxyEventV2){
  ...
  const g=await ObjGet.handle(withTypeId(event,"purchaseOrder",id)); if(g.statusCode!==200||!g.body) return g;
  const po=JSON.parse(g.body);
  if (!["received","partially_received","cancelled"].includes(String(po.status))) return json(409,{message:`Cannot close when status is ${po.status}`});
  po.status="closed";
