...
export async function handle(event: APIGatewayProxyEventV2){
  ...
  const g=await ObjGet.handle(withTypeId(event,"reservation",id)); if(g.statusCode!==200||!g.body) return g;
  const r=JSON.parse(g.body);
  if (!["pending","confirmed","checked_in"].includes(String(r.status))) return json(409,{message:`Cannot start from ${r.status}`});
  r.status="in_use";
  return ObjUpdate.handle(withTypeId(event,"reservation",id,r));
}
