...
export default function OrganizationsListScreen({ navigation }: any) {
  const t = useColors();
  const q = useObjectsList<Organization>({ type: "organization", limit: 20, by: "updatedAt", sort: "desc" });
  ...
  const renderItem = ({ item }: { item: Organization }) => {
    ...
    const title = (item as any).name ?? `Organization ${id.slice(0,8)}`;
    const parts: string[] = [];
    if ((item as any).kind) parts.push(String((item as any).kind));
    if ((item as any).status) parts.push(`Status: ${(item as any).status}`);
    const subtitle = parts.join(" • ") || "—";

    return (
      <Pressable
        onPress={() => navigation.navigate("OrganizationDetail", { id, mode: "edit" })}
  ...
  return (
    <View style={{ flex: 1, backgroundColor: t.colors.background, padding: 12 }}>
      <FlatList
        ...
        ListEmptyComponent={<View style={{ padding: 24 }}>{q.isLoading ? <ActivityIndicator/> : q.isError ? <Text style={{ color: t.colors.danger }}>Error: {String(q.error?.message ?? "unknown")}</Text> : <Text style={{ color: t.colors.muted }}>No organizations.</Text>}</View>}
        contentContainerStyle={{ paddingBottom: 96 }}
      />
      <Pressable
        onPress={() => navigation.navigate("OrganizationDetail", { mode: "new" })}
