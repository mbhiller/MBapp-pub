...
export default function ResourcesListScreen({ navigation }: any) {
  const t = useColors();
  const { data, isLoading, refetch } = Resources.useList({ limit: 20 });
  ...
  return (
    <View style={{ flex: 1, backgroundColor: t.colors.background, padding: 8 }}>
      <FlatList
        ...
        renderItem={({ item }) => (
          <Pressable onPress={() => navigation.navigate("ResourceDetail", { id: item.id })} style={{ padding: 12 }}>
            <Text style={{ color: t.colors.text, fontSize: 16, fontWeight: "700" }}>{item.name || "(unnamed resource)"}</Text>
            <Text style={{ color: t.colors.muted, marginTop: 2 }}>
              {[item.resourceType, item.status].filter(Boolean).join(" • ") || item.id}
            </Text>
          </Pressable>
        )}
      ...
      />
      <Pressable
        onPress={() => navigation.navigate("ResourceDetail", { id: undefined })}
