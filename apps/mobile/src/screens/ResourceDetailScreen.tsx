...
import { useTheme } from "../providers/ThemeProvider";
 ...
 type RoutePropType = RouteProp<RootStackParamList, "ResourceDetail">;
...
export default function ResourceDetailScreen() {
  ...
  const load = async () => {
    if (!resourceId) {
      setLastError("No resource id provided");
      setIsLoading(false);
      return;
    }
    setIsLoading(true);
    ...
    return (
      <View style={{ flex: 1, padding: 16, backgroundColor: t.colors.bg }}>
        ...
        >
          ...
          <Text style={{ color: "#8a1f2d" }}>{lastError || "Resource not found"}</Text>
        </View>
        <Pressable
          onPress={load}
          style={{
    ...
    >
      ...
      >
        <Text style={{ fontSize: 18, fontWeight: "700", color: t.colors.text, marginBottom: 8 }}>
          {(resource as any).name || resource.id || "Resource"}
        </Text>
      ...
      >
        ...
        {renderField("Type", (resource as any).type)}
        {renderField("Status", (resource as any).status)}
        {renderField("Created", formatDateTime((resource as any).createdAt))}
        {renderField("Updated", formatDateTime((resource as any).updatedAt))}
      </View>
    </ScrollView>
  );
}
