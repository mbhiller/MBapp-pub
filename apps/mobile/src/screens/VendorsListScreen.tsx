...
export default function VendorsListScreen({ navigation }: any) {
  const t = useColors();
  const ql = Vendors.useList({ limit: 20 });
    ...
    <View style={{ flex: 1, backgroundColor: t.colors.background, padding: 12 }}>
      <FlatList
        ...
        renderItem={({ item }) => {
          ...
          return (
            <Pressable
              onPress={() => navigation.navigate("VendorDetail", { id: String(item.id), mode: "edit" })}
              style={{
        ...
        ListEmptyComponent={<Text style={{ color: t.colors.muted, textAlign: "center", marginTop: 24 }}>No vendors yet.</Text>}
