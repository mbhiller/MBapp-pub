...
import { useRoles } from "../providers/RolesProvider"; // <-- NEW
...
export default function ModuleHubScreen({ navigation }: Props) {
  const t = useTheme();
  const { allowedModules } = useRoles(); // <-- role-gated list
  ...
  return (
    <View style={sx.container(t)}>
      ...
      <View style={sx.grid(t)}>
        ...
        {/* Utility tile stays visible for now */}
...
const sx = {
  container: (t: ReturnType<typeof useTheme>): ViewStyle => ({
    flex: 1, padding: 16, backgroundColor: t.colors.bg,
  }),
  h1: (t: ReturnType<typeof useTheme>): TextStyle => ({
    fontSize: 22, fontWeight: "700" as TextStyle["fontWeight"], marginBottom: 12, color: t.colors.text,
  }),
  grid: (_t: ReturnType<typeof useTheme>): ViewStyle => ({
    flexDirection: "row", flexWrap: "wrap", justifyContent: "space-between",
  }),
  tile: (t: ReturnType<typeof useTheme>): ViewStyle => ({
    width: "48%", minHeight: 90, borderRadius: 12, padding: 16,
  ...
  tileText: (t: ReturnType<typeof useTheme>): TextStyle => ({
    fontSize: 16, fontWeight: "600" as TextStyle["fontWeight"], color: t.colors.text,
  }),
};
