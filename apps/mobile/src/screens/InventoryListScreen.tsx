...
export default function InventoryListScreen({ navigation }: any) {
  const t = useColors();
  const { data, isLoading, refetch } = Inventory.useList({ limit: 20 });
  ...
  return (
    <View style={{ flex: 1, backgroundColor: t.colors.background, padding: 8 }}>
      <FlatList
        ...
        renderItem={({ item }) => (
          <Pressable onPress={() => navigation.navigate("InventoryDetail", { id: item.id })} style={{ padding: 12 }}>
            <Text style={{ color: t.colors.text, fontSize: 16, fontWeight: "700" }}>{item.name || item.sku || "(unnamed item)"}</Text>
            <Text style={{ color: t.colors.muted, marginTop: 2 }}>{item.price != null ? `$${item.price}` : "good"}</Text>
          </Pressable>
        )}
        ItemSeparatorComponent={() => <View style={{ height: 1, backgroundColor: t.colors.border }} />}
      />
      <Pressable
        onPress={() => navigation.navigate("InventoryDetail", { id: undefined })}
