/* ops/smoke/modules/employees-smoke.mjs */
import { rid, withTag, createObject, listObjects, updateObject, deleteObject } from "../core.mjs";
const TYPE = "employee";
function scaffold(code){ const n=Date.now(); return { type: TYPE, id: rid("emp"), name: withTag(`Employee ${n}`, code), status: "active", role: "general" }; }
...
export async function updateSome({ limit = 5, code } = {}){ const page = await listObjects(TYPE, { limit }); const ids=[]; for(const it of page.items||[]){ const res=await updateObject(TYPE, it.id, { name: withTag(it.name||"Employee", code) }); ids.push(res.id);} return { type: TYPE, updated: ids.length, ids }; }
