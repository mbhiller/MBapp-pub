/* ops/smoke/modules/reservations-smoke.mjs */
import { rid, withTag, createObject, listObjects, updateObject, deleteObject, nowIso } from "../core.mjs";
const TYPE = "reservation";
function addMinutes(dt, m){ return new Date(new Date(dt).getTime()+m*60000).toISOString(); }
function scaffold(code){ const start = nowIso(); return { type: TYPE, id: rid("resv"), name: withTag(`Reservation ${Date.now()}`, code), status: "pending", startsAt: start, endsAt: addMinutes(start, 90) }; }
...
export async function updateSome({ limit = 5, code } = {}){ const page = await listObjects(TYPE, { limit }); const ids=[]; for(const it of page.items||[]){ const res=await updateObject(TYPE, it.id, { name: withTag(it.name||"Reservation", code) }); ids.push(res.id);} return { type: TYPE, updated: ids.length, ids }; }
