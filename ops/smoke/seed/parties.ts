...
async function createParty(api: Api, kind: "person" | "organization", name: string) {
  const res = await api.post(`/objects/party`, {
    type: "party",
    kind,
    name,
  });
  await must(!!res?.ok && !!res.body?.id, () => `createParty failed ${res?.status}: ${JSON.stringify(res?.body)}`);
