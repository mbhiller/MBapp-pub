...
**CI Smoke Manifest:** The definitive list of tests run in CI is maintained in [ops/ci-smokes.json](../ops/ci-smokes.json). Additional flows exist in `ops/smoke/smoke.mjs` but are opt-in only. CI includes `smoke:views:crud`, `smoke:workspaces:list`, `smoke:workspaces:mixed-dedupe`, `smoke:workspaces:get-fallback`, `smoke:views:apply-to-po-list`, `smoke:views:apply-to-product-list`, `smoke:views:apply-to-inventory-list`, and `smoke:views:apply-to-party-list`.
...
**Reliability Hardening (Sprint V/U/J):**
