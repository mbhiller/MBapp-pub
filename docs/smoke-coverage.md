...
- **Messages list + batch retry (simulate):** `smoke:messages:list-and-batch-retry`
  ...
  - Copy semantics: batch retries use stored `subject`/`body` (frozen payload); no template re-render on retry.
...
- **Confirmation Email (templated):** `smoke:registrations:confirmation-message`
  ...
  - **Stable assertions:** Validate template metadata exists (`templateKey`/`templateVars`), not rendered subject/body text (allows copy updates without breaking tests).
  - Flags/Headers: `X-Feature-Registrations-Enabled: true`, `X-Feature-Stripe-Simulate: true`, `X-Feature-Notify-Simulate: true`.
...
- **Confirmation SMS (templated):** `smoke:registrations:confirmation-sms`
  ...
  - **Stable assertions:** Validate template metadata exists (`templateKey`/`templateVars`), not rendered body text (allows copy updates without breaking tests).
  - Flags/Headers: `X-Feature-Registrations-Enabled: true`, `X-Feature-Stripe-Simulate: true`, `X-Feature-Notify-Simulate: true`.
...
- **Background: Retry Failed Messages:** `smoke:jobs:background-retry-failed-messages`
  ...
  - Copy semantics: background retries use stored `subject`/`body` (frozen payload); no template re-render on retry.
...
**CI-covered patch-lines flows (Sprint G, enhanced Sprint V, hardened Sprint AZ E3):**
...
- `smoke:so:patch-lines:cid` — **Sprint AZ E3 (hardened):** Creates SO draft with 1 initial line, performs multi-op patch in single request (add via cid, update via id, remove via id), verifies new line has L{n} id, removed line is gone, no tmp-* ids linger. Then cancels SO and verifies 409 on subsequent patch (status guard). Assertions: `multiOp_ok`, `multiOp_newLineAdded`, `multiOp_removedLineGone`, `multiOp_newLineHasL_n_Id`, `multiOp_noTmpIdsRemain`, `statusGuard_409_on_cancelled`. **Previous behavior:** Single cid→id roundtrip test (still validated).
- `smoke:po:patch-lines:cid` — **Sprint AZ E3 (hardened):** Mirrors SO smoke scope: multi-op patch (add/update/remove), new line L{n} assignment, no tmp-* ids, 409 guard on non-draft (cancelled) status. Assertions: identical to SO smoke. **Spring V enhancements:** Format validation (clientIdValid regex /^tmp-/, serverIdValid regex /^L\d+/), roundtrip validation (cid→server-id flow). **Previous behavior:** Single cid→id roundtrip test (still validated).
