...
**Epic Summary:** Add CI smoke coverage for wipe-tenant.mjs safety guards; validate all 4 negative tests (confirm-tenant match, allowlist, dry-run default) to prevent accidental production data deletion.

- **Wipe Tool Safety Smoke (E1):** [ops/smoke/smoke.mjs](../ops/smoke/smoke.mjs) now includes `smoke:wipe-tool:safety-guards` with 4 test cases:
  1. **Missing --confirm-tenant** → Invokes `wipe-tenant.mjs --tenant SmokeTenant --confirm` (missing `--confirm-tenant`), asserts exit code 2 and error message "--confirm requires --confirm-tenant"
  2. **Mismatched --confirm-tenant** → Invokes with `--confirm-tenant WrongTenant`, asserts exit 2 and "does not match target tenant"
  3. **Non-allowlisted tenant** → Invokes with `--tenant HackerTenant --confirm --confirm-tenant HackerTenant`, asserts exit 2 and "not in allowlist"
  4. **Dry-run succeeds** → Invokes with `--tenant SmokeTenant` (no --confirm), asserts exit 0 and "Dry run complete"
  - All tests use `spawnSync` from child_process to invoke CLI tool and validate exit codes + stderr/stdout
  - **Non-destructive**: No actual deletes performed; all tests validate refusal mechanisms or dry-run mode

- **CI Registration (E2):** [ops/ci-smokes.json](../ops/ci-smokes.json) updated:
  - Added `{ "name": "smoke:wipe-tool:safety-guards", "tier": "core" }`
  - Core tier now has 42 flows (was 41)
  - Positioned after auth foundation smokes, before line-identity smoke

- **Documentation (E3):** 
  - [docs/smoke-coverage.md](smoke-coverage.md): Added `smoke:wipe-tool:safety-guards` entry in Health & Core table with 4 test case descriptions and explicit "Non-destructive" note
  - This section (MBapp-Status.md Sprint AQ): Documents implementation, verification, and Definition of Done
  - **CI-safe adjustment:** Dry-run test (test 4) conditionally runs only when AWS credentials are available; skipped in CI (which intentionally lacks AWS creds). The 3 negative safety tests (confirm-tenant match, allowlist, mismatch rejection) always run and require no AWS access.

- **Verification:**
  - TypeScript compilation clean: `npm run typecheck --workspaces --if-present` ✓
  - Individual smoke test: `node ops/smoke/smoke.mjs smoke:wipe-tool:safety-guards` → PASS (all 4 tests passed)
  - Core suite execution: `npm run smokes:run:core` → All 42 core flows PASS (including new smoke)

- **Definition of Done:**
  - ✅ E1: smoke:wipe-tool:safety-guards implemented with 4 negative test cases
  - ✅ E2: Added to ci-smokes.json core tier (core now 42 flows)
  - ✅ E3: Documentation updated (smoke-coverage.md + MBapp-Status.md)
  - ✅ Typecheck clean, individual smoke passes, core suite passes
  - ✅ No regressions (existing 41 core smokes still pass)
