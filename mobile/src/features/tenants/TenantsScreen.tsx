...
export default function TenantsScreen() {
  ...
  if (isLoading && !data) return <Text style={{ padding: 16 }}>Loading tenants…</Text>;
  if (error) {
  const msg = (error as any)?.message ?? "Unknown error";
  return <Text style={{ padding: 16 }}>Failed to load tenants: {msg}. Pull to retry.</Text>;
}
  ...
  return (
    <>
      <FlatList
        ...
        ListEmptyComponent={<Text style={{ padding: 16 }}>No tenants yet.</Text>}
      />
